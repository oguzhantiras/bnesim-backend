...
app.get("/debug/bnesim-login-compare", async (req, res) => {
  ...
  async function tryLogin(mode) {
    ...
    if (mode === "username") {
      form.append("username", process.env.BNESIM_API_KEY || "");
      form.append("password", process.env.BNESIM_API_SECRET || "");
      form.append("type", "operator");
    }
  ...
  try {
    const a = await tryLogin("api_key");
    const b = await tryLogin("username");
